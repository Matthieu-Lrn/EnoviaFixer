...
'                     SCRIPT VERSION
'*****************************************************************************************************
Private Const sCommonModuleVersion As String = "KBE_GCC_COMMON_APR04_2019"
'*****************************************************************************************************
...
Public Function setCommonVariables()
    ...
    sSettingsPath = sTempDirectory & oPathDict("Saved Settings")
...
'* Language: VBA
'********************************************************************************
Public Function GetTagValueFromFile(ByVal sFilePath As String, ByVal sTag As String, Optional bReset = False) As String
...
'Read file content or retrieve in oDict
If Not oDict.Exists(sFilePath) Then
    sEntireFile = ReadTextFile(sFilePath, 0, True)
    Call oDict.Add(sFilePath, sEntireFile)
Else
    sEntireFile = oDict(sFilePath)
End If
...
Public Sub openPage(ByVal strPartURL As String, Optional ByVal URLLength As Integer)
    ...
    For i = 0 To objShellWindows.Count - 1
        ...
        If Not (objIE Is Nothing) Then
            If InStr(Replace(objIE.LocationURL, "/", "\"), Trim(Left(strPartURL, URLLength))) Then
...
OpenNewWindow:
    ...
    Dim iServerWaitTime
    iServerWaitTime = 500
    Set objxIE = CreateObject("InternetExplorer.Application")
    Err.Clear
    objxIE.Navigate strPartURL
    With objxIE
        Do
            DoEvents
            If Err.Number <> 0 Then Exit Do
            .waitForResponse iServerWaitTime
...
'    Dim lSuccess As Long
   ...
   ' lSuccess = ShellExecute(0, "Open", strPartURL, "", "", 0)
